...
const Question = () => {
  ...
  const fetchDataFromAPI = async () => {
    if (surveyId) {
      ...
      if (result) {
        const trasnformData = result?.responses.map((v) => ({
          content: v.fields[fieldName],
          createdAt: v.createdTime,
        }));
        setData(trasnformData || []);
        setLoading(false);
      }
    } else {
      return setData([]);
  ...
  return (
    <div className="rounded-sm bg-white px-5 pt-6 pb-2.5 shadow-default sm:px-7.5 xl:pb-1">
      ...
      <h1 className="py-5 text-3xl font-semibold text-center">{fieldName}</h1>
              ...
              data.map((item, key) => (
                <tr key={key}>
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 xl:pl-11">
                    <h5 className="text-black">{item.content}</h5>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 xl:pl-11">
                    <h5 className="text-black">{formatDate(item.createdAt)}</h5>
                  </td>
                </tr>
              ))
            ) : (
